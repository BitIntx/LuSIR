...
white markings that bicubic merges into soft blobs. On this sample it improves
Y PSNR from `22.10` to `28.05 dB` (`+5.95 dB`) and Y SSIM from `0.737` to
`0.926` (`+0.189`). This is deliberately a high-impact explanation image, not
a claim that every benchmark image improves by the same amount.

Reproducibility: the GIF uses `Set5/img_003` from the fixed x4 benchmark
manifest. The LuSIR frame is generated by `tools/eval/run_sr_benchmark.py
