...
![Urban100 img_043 deterministic x4 comparison](docs/assets/lusir_current_demo_urban100_img043.jpg)

A high-impact deterministic x4 example from the formal Urban100 benchmark.
Columns are LR input, bicubic, LuSIR masked detail v2, and ground truth, with a
zoom crop underneath. On this sample, LuSIR masked detail v2 improves over
bicubic by `+10.66 dB` Y PSNR and `+0.152` Y SSIM. The example is intentionally
chosen to make the current strength easy to see: repeated architectural
structure is recovered far better than bicubic, while the method still remains
a conservative deterministic restoration path rather than a generative texture
hallucinator.

Reproducibility: the image uses `Urban100/img_043` from the fixed x4 benchmark
manifest. The LuSIR column is generated by `tools/eval/run_sr_benchmark.py
--variant detail_v2_masked` with
