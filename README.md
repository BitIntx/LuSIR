...
The animation shows the same `Set5/img_003` butterfly as noisy LR input, noisy
bicubic, and LuSIR masked detail v2. The deterministic input degradation is
Gaussian noise with `sigma=4/255` and seed `20260622`; the first frame uses
nearest-neighbor enlargement only to make the actual LR pixels visible. Both
bicubic and LuSIR receive that same noisy LR. Against the clean GT, LuSIR
improves Y PSNR from `22.07` to `27.24 dB` (`+5.17 dB`) and Y SSIM from `0.730`
to `0.893` (`+0.163`). It restores wing outlines and small white markings that
the noisy bicubic baseline merges into soft blobs. This is deliberately a
high-impact explanation image, not a claim that every benchmark image improves
by the same amount.

Reproducibility: the GIF uses `Set5/img_003` from the fixed x4 benchmark
manifest and the disclosed Gaussian degradation above. The LuSIR frame is
generated by `tools/eval/run_sr_benchmark.py --variant detail_v2_masked` with
`configs/hf/detail_branch_v2_masked_photo130k_lsdir.yaml`; the corresponding
output pattern is `benchmark_outputs/detail_v2_masked/{dataset}/{id}/detail.png`.
The machine-readable comparison is preserved as
`metrics/formal_x4_benchmark_stage2_latent_adapter_v1_value_compare_metrics.csv`.
The public Colab default remains the lighter guarded-detail Stage 2 v2 path;
masked detail v2 is exposed as a research/detail option.
