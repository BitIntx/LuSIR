...
3. `configs/detail_branch_v8_gtmask_training_probe.yaml`도 완료 후 step2000에서
   조기 중단했다.
   ...
   step1750도 PSNR delta `+0.1000`, SSIM delta `+0.00429`, highpass ratio delta
   `+0.0151`, laplacian ratio delta `+0.0019`로 안정적이었다. 하지만 step2000은
   PSNR delta `+0.1008`에도 laplacian ratio delta가 `-0.0006`으로 꺾였고,
   best detail score는 step500에 머물렀다. best checkpoint는
...
4. Stage2 GT-masked detail v3 probe도 step1000 eval 뒤 조기 중단했다.
   ...
   guarded-detail Stage2 v2 best10000에서 시작하고, train-only
...
- 다음 subset-scale probe는
  ...
  <https://wandb.ai/jwheo/LuSIR/runs/0wzx4xzy>이다.
...
7. 다음 생성형 detail 실험은 LR에서 근거가 있는 위치만 여는 learned
   uncertainty/detail mask와 patch-level perceptual/adversarial supervision을
   결합하는 two-head 구조를 우선 검토한다.
8. clean-fidelity gap 개선은 별도 Stage2/base 구조 연구로 유지하고,
   생성형 detail 목표와 섞지 않는다.
9. public Colab 기본 경로는 guarded-detail Stage2 v2 step10000으로 변경했다.
   tile batch size 기본값은 T4 안정성을 위해 `1`이다. residual refiner v2,
   detail branch v1d, masked v2는 WebUI 옵션으로 유지한다.
10. `detail-need mask` GT target/proxy/진단 metric 구현과 photo-detail val100
...
11. learned mask predictor와 masked detail branch v2 검증은 완료됐다. selected
   step38000은 ordinary val100에서 PSNR `+0.18177 dB`, SSIM `+0.00755`,
   wins `100/100`이지만 시각적으로 v1d와 거의 같고 step50000까지 plateau했다.
12. masked v2 추론/benchmark 재현 경로와 HF preset은 추가됐다. formal
...
13. 다음 detail 실험은 같은 objective continuation이 아니라 frozen fidelity base와
...
14. Stage1 audit 이후 Stage2 guarded-detail v2 probe를 추가했다:
