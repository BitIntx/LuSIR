...
\NormalTok{metrics/stage2\_bridge\_guarded\_interpolation\_sweep\_val100.json}
\NormalTok{metrics/formal\_x4\_benchmark\_stage2\_bridge\_guarded\_soup\_a025\_summary.json}
\NormalTok{metrics/formal\_x4\_benchmark\_stage2\_bridge\_guarded\_soup\_a025\_metrics.csv}
\end{Highlighting}
\end{Shaded}
...
V2 was stopped after step1000. SSIM increased from \texttt{0.82145} to
\texttt{0.82921} and missing energy decreased from \texttt{0.01798} to
\texttt{0.01533}, confirming that the target-aligned gradient restores
structural high-frequency response. The weight was too strong: highpass
ratio increased from \texttt{0.82468} to \texttt{0.90093}, excess energy
from \texttt{0.00679} to \texttt{0.00915}, and mean PSNR fell by
\texttt{0.0510\ dB}.

V3 isolates the balance rather than changing architecture or data again.
It uses excess weight \texttt{1.5} and missing weight \texttt{0.8},
interpolated between the V1 under-detail and V2 over-detail boundaries.
All other training and evaluation settings remain fixed.

V3 completed 12000 steps and selected step11500 by held-out
clean-bicubic val100 mean PSNR. Relative to the initial dual-context
checkpoint, mean PSNR increases from \texttt{26.91987} to
\texttt{27.05483\ dB}, SSIM from \texttt{0.82145} to \texttt{0.82665},
highpass L1 decreases from \texttt{0.031130} to \texttt{0.030733}, and
missing energy decreases from \texttt{0.017983} to \texttt{0.017283}.
The selected grid shows no obvious ripple pattern or oversharpening.
...
\bottomrule\noalign{}
\endlastfoot
Dual-context step98000 & \texttt{27.84314} & \texttt{0.79742} &
\texttt{26.31306} & \texttt{0.77340} \\
Generalization V3 step11500 & \textbf{\texttt{27.99167}} &
\textbf{\texttt{0.80295}} & \textbf{\texttt{26.47050}} &
\textbf{\texttt{0.77969}} \\
\end{longtable}

This result does not transfer to real-degradation robustness. Relative
to step98000, decoded PSNR regresses by \texttt{0.2125\ dB} on
\texttt{mild}, \texttt{0.2749\ dB} on \texttt{photo\_detail\_mix},
\texttt{0.9644\ dB} on \texttt{photo\_v2}, and \texttt{0.7808\ dB} on
\texttt{photo\_v3\_noise\_mix}. V3 is therefore preserved as the
strongest standalone clean-bicubic Stage 2 research checkpoint, but it
does not replace the public HF or Colab default. A follow-up must mix
clean and progressively stronger degradations while retaining both
formal clean metrics and all four cross-preset evaluations as
guardrails.

To test whether residual clean-fidelity error is capacity limited, the
next probe expands only the full-resolution residual trunk from 16 to 40
blocks. This raises Stage 2 capacity from \texttt{119.238M} to
\texttt{147.587M} parameters (\texttt{1.238x}) while leaving both
multiscale context branches unchanged. The new blocks use
zero-initialized second convolutions, so partial initialization from V3
step11500 reproduces the original output exactly before training. The
4000-step probe evaluates clean bicubic plus \texttt{mild},
\texttt{photo\_detail\_mix}, \texttt{photo\_v2}, and
\texttt{photo\_v3\_noise\_mix} every 500 steps. On one L40S, batch 6
without gradient accumulation is the largest stable configuration across
training and all evaluations, using approximately \texttt{39.2/46.1GB}
VRAM. A clean gain below \texttt{0.05\ dB}, or renewed
degradation-preset regression, is evidence that capacity is not the
primary bottleneck.

The probe completed 4000 steps and selected step3500. Held-out
clean-bicubic val100 mean PSNR improved by only \texttt{0.00364\ dB}.
All robustness evaluations regressed: \texttt{-0.02011\ dB} on
\texttt{mild}, \texttt{-0.01005\ dB} on \texttt{photo\_detail\_mix},
\texttt{-0.05466\ dB} on \texttt{photo\_v2}, and \texttt{-0.06268\ dB}
on \texttt{photo\_v3\_noise\_mix}.
...
\bottomrule\noalign{}
\endlastfoot
Generalization V3 step11500 & \texttt{27.99167} & \texttt{0.802953} &
\texttt{26.47050} & \texttt{0.779686} \\
148M trunk step3500 & \textbf{\texttt{27.99716}} &
\textbf{\texttt{0.803327}} & \textbf{\texttt{26.47654}} &
\textbf{\texttt{0.780054}} \\
\end{longtable}

The Y-PSNR gain is \texttt{0.00549\ dB} with only \texttt{114/219}
per-image wins. DIV2K improves by \texttt{0.00189\ dB} while winning
only \texttt{44/100} images. Fixed grids and the formal contact sheet
show no distinguishable visual improvement. The additional 28.35M
parameters and higher memory cost are therefore not justified. The
result rejects simple Stage 2 trunk capacity as the primary bottleneck
and redirects the next experiment toward a mixed clean/real degradation
curriculum.

The follow-up returns to the selected \texttt{119.238M} V3 architecture
and changes the training distribution instead of model capacity. The
robustness bridge mix contains \texttt{55\%} benchmark bicubic,
\texttt{20\%} restrained photo-detail degradation, \texttt{15\%} mild,
\texttt{8\%} photo-v2, and \texttt{2\%} photo-v3 noise. It starts from
V3 step11500, uses a \texttt{2e-6} warmup-cosine learning rate, batch 8
without gradient accumulation, and runs for at most 6000 steps.

Checkpoint selection uses a weighted PSNR score over clean
(\texttt{45\%}), mild (\texttt{15\%}), photo-detail-mix (\texttt{15\%}),
photo-v2 (\texttt{12.5\%}), and photo-v3-noise-mix (\texttt{12.5\%}). A
candidate is invalid unless clean mean PSNR is at least
\texttt{27.02\ dB}, clean SSIM at least \texttt{0.8260}, and clean
excess energy at most \texttt{0.0075}. The trainer now supports a
primary \texttt{eval.data} override so the clean benchmark remains
independent of the mixed training preset. The initial CUDA smoke
reproduces clean mean PSNR \texttt{27.0550}, obtains composite score
\texttt{25.7430}, passes all guardrails, and the full 97-test suite
passes.

Bridge v1 completed 6000 steps. At step500, clean mean PSNR regressed by
\texttt{0.03873\ dB}, while mild and photo-detail-mix improved by
\texttt{0.27055} and \texttt{0.26293\ dB}; photo-v2 and
photo-v3-noise-mix improved by \texttt{0.77224} and
\texttt{0.72275\ dB}. This missed the clean threshold by only
\texttt{0.00372\ dB}, but later steps did not improve the trade-off.
Step4500 had the highest raw composite score while losing
\texttt{0.06485\ dB} clean fidelity. No trained checkpoint passed all
guardrails.

Bridge v2 therefore raises the clean share from \texttt{55\%} to
\texttt{70\%}, reduces the combined strong-degradation share from
\texttt{10\%} to \texttt{5\%}, halves the learning rate to
\texttt{1e-6}, and limits training to 1500 steps with 250-step
evaluation. Its mix is \texttt{70\%} benchmark bicubic, \texttt{15\%}
photo detail, \texttt{10\%} mild, \texttt{4\%} photo-v2, and
\texttt{1\%} photo-v3 noise. Selection and guardrails remain unchanged
for direct comparison. The initial smoke reproduces composite score
\texttt{25.74284}, clean mean PSNR \texttt{27.05493}, and passes the
full 98-test suite. The live run is tracked at
\url{https://wandb.ai/jwheo/LuSIR/runs/i3txwnqz}. Its first trained
candidate at step250 passes all guardrails: clean mean PSNR is
\texttt{27.03346} (\texttt{-0.02160\ dB}), SSIM is \texttt{0.82706}, and
excess energy is \texttt{0.007321}. Relative to initialization, mild and
photo-detail-mix improve by \texttt{0.18215} and \texttt{0.15581\ dB},
while photo-v2 and photo-v3-noise-mix improve by \texttt{0.73252} and
\texttt{0.59429\ dB}. This is the first mixed-degradation continuation
to retain the selected V3 model\textquotesingle s clean guardrail while
recovering substantial robustness.

The run completed 1500 steps and selected step1000 with composite score
\texttt{25.99427}. Relative to V3 initialization, clean val100 mean PSNR
changes by \texttt{-0.01925\ dB}, while mild, photo-detail-mix,
photo-v2, and photo-v3-noise-mix improve by \texttt{0.24575},
\texttt{0.22719}, \texttt{0.80926}, and \texttt{0.70348\ dB}. On the
formal 219-image clean-bicubic benchmark, bridge v2 reaches
\texttt{27.97010} Y PSNR and \texttt{0.80259} Y SSIM. This is only
\texttt{0.02156\ dB} and \texttt{0.000366} below V3, while remaining
\texttt{0.12697\ dB} and \texttt{0.00517} above dual best98000.

The public guarded-detail v2 checkpoint remains the stronger
degraded-input default. On the same five val100 sets, bridge v2 gains
\texttt{0.05825\ dB} clean decoded PSNR but trails guarded v2 by
\texttt{0.04087}, \texttt{0.06238}, \texttt{0.04268}, and
\texttt{0.09853\ dB} on mild, photo-detail-mix, photo-v2, and
photo-v3-noise-mix. Bridge v2 is therefore preserved as a balanced
clean/robustness research checkpoint rather than replacing the public
default.

We also tested a no-training model soup between bridge v2 best1000 and
guarded v2 best10000. The sweep linearly interpolates all compatible
Stage 2 weights; alpha denotes the guarded-v2 fraction. On the same five
val100 presets, alpha \texttt{0.25} is the best valid point: clean mean
PSNR drops only \texttt{0.00833\ dB} from bridge v2, while mild,
photo-detail-mix, photo-v2, and photo-v3-noise-mix increase by
\texttt{0.03215}, \texttt{0.03590}, \texttt{0.01612}, and
\texttt{0.03029\ dB}. The raw composite score rises from
\texttt{25.99427} to \texttt{26.00653}. However, the formal 219-image
clean-bicubic benchmark falls to \texttt{27.96126} Y PSNR and
\texttt{0.80233} Y SSIM, which is \texttt{0.00884\ dB} and
\texttt{0.000257} below bridge v2. The soup is therefore useful evidence
for the clean/robustness trade-off, but it does not replace either the
bridge research checkpoint or the public guarded default.
