...
The current README demo uses an animated \texttt{Set5/img\_003}
bicubic-to-LuSIR toggle, where masked detail v2 improves Y PSNR by
\texttt{+5.95\ dB} and Y SSIM by \texttt{+0.189}. Because the PDF cannot
animate that comparison, the static report figure below uses
\texttt{Urban100/img\_043}, the strongest measured full-image example
from the fixed x4 benchmark among the reviewed candidates. It is
intentionally used as a high-impact visual explanation rather than as a
claim of average SOTA quality: masked detail v2 improves this sample
over bicubic by \texttt{+10.66\ dB} Y PSNR and \texttt{+0.152} Y SSIM,
mainly by restoring repeated architectural structure.
