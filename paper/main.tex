...
Snapshot: dual-context Stage 2 scale-up complete and 3.02M-parameter
detail branch v1d capacity run in progress.
...
\NormalTok{  LR {-}\textgreater{} dual{-}context LSDIR Stage 2 {-}\textgreater{} Stage 1 decoder {-}\textgreater{} detail branch v1d}
...
Dual-context + detail v1d, in progress & step 9500 & \texttt{143.354M} &
\texttt{31.8247} & \texttt{+2.2248} \\
...
\texttt{+0.0744\ dB}, and the in-progress 3.02M v1d
\texttt{+0.0837\ dB}. V1d currently improves v1c by only
\texttt{+0.0093\ dB}, so increasing detail-branch capacity alone is not
yet a perceptual-detail breakthrough.
...
used by many SR papers. A formal comparison still requires full DIV2K,
Set5, Set14, and Urban100 evaluation under each
benchmark\textquotesingle s exact protocol.
...
\includegraphics{../docs/assets/detail_branch_v1b_lime_demo.jpg}

\emph{Figure 2. Detail branch v1b adds a small, artifact-light
correction over the base; this illustrates both its stability and its
...
\texttt{98/100} wins. The selected step 39500 remains the latest
preserved public detail artifact because it has the best combined detail
...
below GT on fine surface detail. It is not yet the public Colab default
because the WebUI currently wraps the residual-refiner and diffusion
single-image runners.
...
output exactly reproduces v1c. The run is configured for 100,086
micro-steps, exactly three passes over the 133,450-image manifest at
batch size 4. It remains in progress. At the strict-bicubic snapshot
selected step 9500, it reaches \texttt{31.8247\ dB}, only
\texttt{+0.0093\ dB} above v1c. On the ordinary fixed
\texttt{photo\_detail\_mix} val100 it reaches \texttt{+0.0638\ dB}
aggregate PSNR and \texttt{+0.00337} SSIM over the frozen base with
\texttt{100/100} wins. This is a real but still incremental gain; the
larger branch has not yet resolved the visible fine-texture gap.
...
The selected residual refiner remains the public Colab default until the
detail branch has a single-image inference runner and WebUI integration.
The completed perceptual Stage 2 continuation is not promoted. Candidate
next steps are:

\begin{itemize}
\tightlist
\item
  continue v1d to at least one epoch unless fixed validation or visual
  quality clearly regresses, then compare its selected checkpoint
  against v1b/v1c;
\item
  build the formal full-image DIV2K, Set5, Set14, and Urban100 benchmark
  with Y-channel PSNR, border shave, and benchmark-compatible bicubic
  generation;
\item
  run Real-ESRGAN and other public baselines on the same fixed visual
  set and add LPIPS, DISTS, and blind human comparison rather than
  relying on PSNR alone;
\item
  if v1d remains visually conservative, change the supervision toward
  perceptual/adversarial detail recovery instead of increasing branch
  capacity again;
\item
