...
any cached sample, so the active v8 probe removes teacher supervision
and uses GT detail-need masks only during training.
...
\NormalTok{active GT{-}mask training probe:}
...
\item
  run and review the v6 no-GAN detail branch probe first, watching W\&B
  sample grids, \texttt{eval/sr\_vs\_base\_mean\_psnr},
  \texttt{eval/sr\_vs\_base\_ssim}, \texttt{eval/lowpass\_drift\_l1},
  \texttt{eval/outside\_mask\_residual\_l1},
  \texttt{train/negative\_residual}, and
  \texttt{train/negative\_weight}; this is now complete and is recorded
  as a safe but non-improving result;
\item
