...
The current README demo uses an animated \texttt{Set5/img\_003} noisy-LR
to bicubic to LuSIR sequence. A fixed Gaussian degradation
(\texttt{sigma=4/255}, seed \texttt{20260622}) is applied before both
bicubic and LuSIR inference. Masked detail v2 improves over that noisy
bicubic baseline by \texttt{+5.17\ dB} Y PSNR and \texttt{+0.163} Y
SSIM. Because the PDF cannot animate that comparison, the static report
figure below uses \texttt{Urban100/img\_043}, the strongest measured
clean-bicubic full-image example from the fixed x4 benchmark among the
reviewed candidates. It is intentionally used as a high-impact visual
explanation rather than as a claim of average SOTA quality: masked
detail v2 improves this sample over bicubic by \texttt{+10.66\ dB} Y
PSNR and \texttt{+0.152} Y SSIM, mainly by restoring repeated
