...
Snapshot: formal x4 benchmark complete, Stage 2 clean-fidelity
learning-rate probes complete, and a separate high-frequency residual
diffusion path selected as the next generative experiment.
...
Y-channel results. The formal DIV2K comparison remains LuSIR detail v1d
at \texttt{30.1602\ dB} and SwinIR classical x4 at \texttt{31.0838\ dB}.
...
detail branch v1d is available as a selectable single-image/tiled Colab
research option. The completed perceptual Stage 2 continuation is not
promoted. Candidate next steps are:

\begin{itemize}
\tightlist
\item
  preserve the deterministic base as the fidelity path and train a
  separate gated, bounded high-frequency residual diffusion model for
...
\item
  because v1d remains visually conservative, consider detail-only
  perceptual or adversarial supervision instead of increasing branch
  capacity again;
\item
...
representation can express useful corrections. The step-3000
condition-start model is not yet promoted: start timesteps 15, 25, and
50 trail the base by \texttt{0.5768}, \texttt{1.2256}, and
\texttt{3.6719} dB respectively, and stronger settings retain visible
stochastic grain. Because this probe contains only 375 optimizer updates
and its signed-wavelet error is still decreasing, a step-20000
continuation is running before the architecture is rejected.
